...
describe('psuedo content', () => {
  ...
  it('should keep the whole content in the correct order', () => {
    testElement.setAttribute('data-string', 'foo');
    style.innerHTML = `div:before { content: attr(data-string) 'bar' counter(bar) ' attr(data-string) " '; }`;
    expect(getPseudoContent(testElement, '::before')).to.equal('foobar attr(data-string) " ');
  });

  it('should handle escaped characters correctliy', () => {
    style.innerHTML = `div:before { content: "'" '"' '\\\\' '\\A';} `;
    expect(getPseudoContent(testElement, '::before')).to.equal("'\"\\");
  });

  it('should handle white space correctly when applicable', () => {
    testElement.style.whiteSpace = 'pre';
    testElement.setAttribute('data-string', `a
b`);
